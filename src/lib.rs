...
fn main() {
    ...
    match result {
        ...
        Err(err) => {
            println!("ERROR - Unable to read directory {}: {}", patch_dir, err);
            return;
        }
    }
    ...
    for entry in entries {
        ...
        if !path.is_file() || path.extension().unwrap_or_default() != "1337" {
            continue;
        }
        let content = fs::read_to_string(&path)
            .unwrap_or_else(|_| panic!("ERROR - Failed to read file: {}", &path.to_str().unwrap()));
        ...
        match result {
            Ok(_) => continue,
            Err(err) => {
                println!("ERROR - Failed to apply patch: {}", err);
                continue;
            }
        };
    }
